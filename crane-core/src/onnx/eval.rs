...
fn simple_eval_(
    ...
    for node in graph.node.iter() {
        ...
        match node.op_type.as_str() {
            ...
            // Crane Added 20260731: implementation lives in ops/lstm.rs.
            "LSTM" => {
                let outputs = ops::lstm::lstm(node, ops::lstm::LstmInputs {
                    input: get(&node.input[0])?,
                    weight: get(&node.input[1])?,
                    recurrence_weight: get(&node.input[2])?,
                    bias: get_opt(3).transpose()?,
                    seq_lens: get_opt(4).transpose()?,
                    initial_h: get_opt(5).transpose()?,
                    initial_c: get_opt(6).transpose()?,
                    peephole: get_opt(7).transpose()?,
                })?;
                if let Some(y) = outputs.y
                    && let Some(name) = node.output.first().filter(|s| !s.is_empty())
                {
                    values.insert(name.clone(), y);
                }
                if let Some(y_h) = outputs.y_h
                    && let Some(name) = node.output.get(1).filter(|s| !s.is_empty())
                {
                    values.insert(name.clone(), y_h);
                }
                if let Some(y_c) = outputs.y_c
                    && let Some(name) = node.output.get(2).filter(|s| !s.is_empty())
                {
                    values.insert(name.clone(), y_c);
                }
            },
            "RNN" => {
            ...
            // Crane Added 20260731: implementation lives in ops/resize.rs.
            "Resize" => {
                let input = get(&node.input[0])?;
                ...
                let output = ops::resize::resize(node, input, scales, sizes)?;
                values.insert(node.output[0].clone(), output);
            },
            // Crane Added 20260731: implementation lives in ops/trilu.rs.
            "Trilu" => {
                let input = get(&node.input[0])?;
                let k_input = get_opt(1).transpose()?;
                let output = ops::trilu::trilu(node, input, k_input)?;
                values.insert(node.output[0].clone(), output);
            },
            "ScatterND" => {
                let data = get(&node.input[0])?;
