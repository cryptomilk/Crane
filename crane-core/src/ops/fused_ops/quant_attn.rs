...
// Only called from `dispatch_qk`'s cfg(not(cuda, rocm)) arm; unused (by
// design) when a GPU backend feature is enabled.
#[cfg_attr(any(feature = "cuda", feature = "rocm"), allow(dead_code))]
...
// Only called from `dispatch_sv`'s cfg(not(cuda, rocm)) arm; unused (by
// design) when a GPU backend feature is enabled.
#[cfg_attr(any(feature = "cuda", feature = "rocm"), allow(dead_code))]
fn cpu_quant_sv_dot(
    weights: &Tensor,
